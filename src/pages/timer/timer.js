...
export default function Timer()
{
            ...
            const clearAllSavedtimer = ()=>{
                setSavedTimer([]);
            }
            const clearTimer =(index)=>{
                const newSavedTimer = [...savedTimer];
                newSavedTimer.splice(index,1);
                setSavedTimer(newSavedTimer);
            }
            ...
            <div className='save-timer'>
                ...
                <ul>
                    {savedTimer.map((time,index)=>(
                        <li key={index}>
                            <a>{time.title}: {formatTimer(time.time)}</a>
                            <button onClick={()=> clearTimer(index)} className='save-timer-partbtn'>삭제</button>
                        </li>
                    ))}
                </ul>
                <br/>
                <button className='save-timer-allbtn' onClick={clearAllSavedtimer}>전체 삭제</button>
            </div>
        </div>
    )
}
